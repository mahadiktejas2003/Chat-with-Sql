...
load_dotenv(r"D:\Generative AI Course CampusX\Chat with SQL project complete\genai_using_llamaIndex\dotenv.env")
class ChatWithSql:
    ...
    def message(self, query):
        ...
        custom_prompt = (
            f"Please provide a precise and accurate response to the following SQL query:\n{query}\n"
            "Make sure to include the numerical result in your answer if applicable."
        )
        response = agent_executor.run(custom_prompt)
        
        # Post-processing the response to ensure it contains a numerical result
        if not any(char.isdigit() for char in response):
            return "The response from the AI was not sufficient. Please try rephrasing your query."
